...
/*
 * mword_new_word — 将单词转为小写
 * @word: 字符串数组
 * 
 * 通过使用strlwr（）函数将单词转为小写，并存于tmp里，便于下面排序
 * 
 * 返回值：指向对应的结构体的指针
 */
static struct word_s *mwd_new_word(char word[]) 
{
    ...
    tmp = (struct word_s *)malloc(LEN);       /* 给一个新的节点申请空间 */
    if (tmp == NULL) {
        (void)printf("内存分配失败！\n");
        return NULL;
    }
 ...
 * 返回值：排完序的链表头结点
 */
static struct word_s *mwd_instert_word(struct word_s *temp, struct word_s *head)
{
    struct word_s *cur;
    ...
    if (head == NULL) {         /* 存放在第一个结点 */    
        ...
        while (cur != NULL) {
            ...
            if (strcmp(temp->word, cur->word) < 0) {              
                ...
                break;
                /* 等于情况，单词保留一个，个数加一 */  
            } else if (strcmp(temp->word, cur->word) == 0) {       
                cur->num++;
                free(temp);        
                break;
            } else if (strcmp(temp->word, cur->word) > 0) {      /* 大于情况， */ 
                if (cur->next != NULL) {    /* 后面不为空情况 */
                    /* 小于下一个值 ,插入当前节点之后 */ 
                    if (strcmp(temp->word, cur->next->word) < 0) { 
                        temp->next = cur->next;
                        cur->next = temp;
                        break; 
                    }
    ...
    while (!feof(input)) {      /* 判断文件是否读完 */
        ...
        if (isalpha(ch)){       /* 判断字符是否是字母 */ 
            ...
            if (strlen(word) != 0) {     /* 将单词插入链表 */
                ...
                head = mwd_instert_word(temp, head);
            }   
            i = 0;              /* 将单词重新从数组首位存放 */
        }
    }
