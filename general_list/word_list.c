...
int main(void)
{
    ...
    struct word_s head;     /* 头结点 */
    INIT_LIST_HEAD(&head.list); /* 初始化 */
...
static struct word_s mwd_first_word(char *word, struct word_s head)
{
    ...
    if (head == NULL) {         
        ...
        if (tmp == NULL) {
            ...
            tmp = mwd_new_word(word);      /* 存放在第一个结点 */
...
static struct word_s mwd_sort_and_cound(char *word, struct word_s head)
{
    ...
    while (cur != NULL) {
        ...
        list_for_each(pos->next, &(head.list))
        { 
            /* 这里我们用list_entry来取得pos所在的结构的指针 */ 
            tmp = list_entry(pos->next, struct word_s, list); 
        }

        /* 小于情况，单词顺序不变 */
        if (stricmp(word, tmp->word) < 0) {
            tmp = mwd_new_word(word);	/* 生成新结点 */
            list_add(&(tmp->list), &(head.list)); /* 头插 */
            break;
            /* 等于情况，单词保留一个，个数加一 */
        } else if (stricmp(word, tmp->word) == 0) {
            tmp->num++;
            break;
        } else if (stricmp(word, tmp->word) > 0) {  /* 大于情况，插cur后面 */
        //    if (cur->next != NULL) {    			/* 后面不为空情况 */
            
            list_for_each(pos->next->next, &(head.list))
            { 
                /* 这里我们用list_entry来取得pos所在的结构的指针 */ 
                tmp = list_entry(pos->next->next, struct word_s, list); 
            }
