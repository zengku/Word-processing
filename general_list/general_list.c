...
static struct word_s *mwd_sort_and_cound(char *word, struct word_s *head)
{
    ...
    cur = (head->list).next;                /* 第一个结点（头结点的下一个结点） */
    
    while (cur != &(head->list)) {          /* cur不是头结点 */
        ...
        if (stricmp(word, tmp->word) < 0) {                 /* 小于情况，单词顺序不变插入 */
            ...
            cur_next = cur->next;
            if (cur_next != &(head->list)) {    			/* 后面不为头结点，接着和后面的比较 */
...
static struct word_s *mwd_collect_words(FILE *input)
{
    ...
    INIT_LIST_HEAD(&(head->list));  /* 初始化头节点 */ 
    ...
    while (!feof(input)) {      /* 判断文件是否读完 */
        ...
        if (isalpha(ch)) {      /* 判断字符是否是字母 */
            ...
            if (strlen(word) != 0) {     /* 将单词插入到链表 */
                if (list_empty(&(head->list))) {            /* 链表为空，将第一个直接单词插入 */
                    tmp = mwd_new_word(word);    
                    list_add(&(tmp->list), &(head->list));  
                } else {
                    head = mwd_sort_and_cound(word, head);  /* 不为空，将单词排序 */
                }
            }
            i = 0;      /* 将单词重新从数组首位存放 */
        }
...
static void mwd_print_file(struct word_s *head)
{
    ...
    pos = &(head->list);
    list_for_next_each(pos, &(head->list)) {            /* 从头结点开始遍历，打印并且写到文件 */
...
static void mwd_destory_list(struct word_s *head)
{
    ...
    cur = (head->list).next;
    while(cur != &(head->list)) {                       /* 遍历头结点，先删除单词的内存，在释放结点 */
        tmp = list_entry(cur, struct word_s, list);
        if (tmp->word) {
            free(tmp->word);
        }
